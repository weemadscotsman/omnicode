...
> **Update (2026-06-01, later pass):** Several rows below were actioned in the engine-v2 doc sweep —
> `ARCHITECTURE.md`, `AGENT_HOOKS.md`, and `AGENT_INSTALL_UNIVERSAL.md` are now current (rows updated
> in place). New canonical record: [`/CHANGELOG.md`](../../CHANGELOG.md). Benchmark/report docs and
> the `release/` bundle now carry point-in-time / frozen-snapshot banners.
...
| `AGENT_HOOKS.md` | ✅ Current — already starts agents with `session_resume_brief` and documents compressed mode (5 public tools, `get_tool_schema` + `invoke_tool`). | Done. |
| `AGENT_INSTALL_UNIVERSAL.md` | ✅ Current — trust-layer order (`session_resume_brief` → `list_tools` → `get_tool_schema` → `invoke_tool`) and compressed-mode caveat already present. | Done. |
| `ARCHITECTURE.md` | ✅ Updated 2026-06-01 — now describes WRR/BM25 ranked retrieval (not Fuse), real PageRank, the worker pool, archive ingestion, optional watcher, and the current storage model. | Done; revisit if the tool registry/output-budget sections warrant their own page. |
