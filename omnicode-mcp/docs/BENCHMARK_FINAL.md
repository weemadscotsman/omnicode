...
> **📌 Point-in-time record.** The numbers below are from a specific prior benchmark run and are
> left intact as a reproducible record. They are **not** re-asserted for the current build. For an
> independently re-verified side-by-side on a fresh 4-repo set (measured payload vs. estimated raw,
> capped at 99.9), see [`/benchmarks/V2_SIDE_BY_SIDE.md`](../../benchmarks/V2_SIDE_BY_SIDE.md). For
> engine changes since this run, see [`/CHANGELOG.md`](../../CHANGELOG.md). To refresh these exact
> headline figures, re-run `omnicode bench-many` / `omnicode sweep` and record what it outputs —
> numbers are never hand-edited into this file.
