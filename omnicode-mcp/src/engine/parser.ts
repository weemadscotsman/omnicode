...
export interface ParseResult {
  ...
  imports: string[];
  blindspots: string[];
  parserMode: 'tree-sitter' | 'fallback' | 'none';
  parseQuality: number;
  languageName: string;
}
...
function makeBlindspot(code: string, severity: 'info'|'warn'|'error', message: string, hint = '') {
...
function isValidSymbolName(name: string): boolean {
  ...
  if (RESERVED_SYMBOL_NAMES.has(n)) return false;
...
class LanguageRegistry {
  ...
  readonly fallbackLanguages = ['typescript','tsx','javascript','python','rust','go','c_sharp','java','kotlin','php','ruby','swift','c','cpp','solidity','lua'];
  ...
  private tryLoad(lang: string, pkg: string, extractor?: (p: any) => any) {
    try {
  ...
  getLanguage(ext: string) { return this.parsers[this.getLanguageName(ext)] ?? null; }
  getLanguageName(ext: string): string { return EXT_TO_LANGUAGE[ext] ?? 'unknown'; }
  isLoadedLanguageName(languageName: string): boolean { return this.loadedLanguages.includes(languageName); }
  hasFallback(languageName: string): boolean { return this.fallbackLanguages.includes(languageName); }
}

export const langRegistry = new LanguageRegistry();

function lineFromOffset(content: string, offset: number) {
  return content.slice(0, Math.max(0, offset)).split('\n').length;
}

function snippetForLine(content: string, start: number, end: number) {
...
function symbolFromRegex(content: string, name: string, kind: string, start: number, endHint?: number): ParsedSymbol {
...
function extractImportsFallback(language: string, content: string): string[] {
  ...
  for (const p of patterns) {
    ...
    while ((m = p.exec(content))) if (m[1]) imports.push(m[1].trim());
  }
  return uniq(imports);
}

function extractSymbolsFallback(language: string, content: string): ParsedSymbol[] {
  const out: ParsedSymbol[] = [];
  const pushMatches = (kind: string, re: RegExp, group = 1) => {
    let m: RegExpExecArray | null;
  ...
  return out.filter((s) => {
    if (!isValidSymbolName(s.name)) return false; // reject keywords / control flow
...
function extractCallsFallback(language: string, content: string, symbols: ParsedSymbol[]): ParsedCallEdge[] {
  ...
  while ((m = callRe.exec(content))) {
    const name = m[1];
    if (!isValidSymbolName(name) || name === 'require' || name === 'import') continue;
    let owner: string | null = null;
    for (const s of sorted) {
      if (s.byteStart <= m.index && s.byteEnd >= m.index) { owner = s.name; break; }
      if (s.byteStart <= m.index) owner = s.name;
    }
...
function fallbackParse(filePath: string, content: string, languageName: string, reason: string): ParseResult {
  ...
  if (reason) blindspots.push(makeBlindspot('DEGRADED_PARSE', symbols.length ? 'warn' : 'error', reason, 'install native parser or use LSP fallback for semantic resolution'));
  if (hasDynamicSignals) blindspots.push(makeBlindspot('DYNAMIC_RUNTIME_REFERENCE', 'warn', 'runtime/dynamic references detected', 'use runtime_trace or LSP/ABI resolver before deleting/renaming'));
...
export function parseSource(filePath: string, content: string): ParseResult {
  ...
  const languageName = langRegistry.getLanguageName(ext);
  const langConfig = langRegistry.getLanguage(ext);

  if (!langConfig) {
    if (langRegistry.hasFallback(languageName)) {
  ...
  try {
    ...
    const walk = (node: any) => {
      ...
      if (isDecl && node.type !== 'variable_declaration') {
        ...
        if (nameNode && valueNode && (valueNode.type === 'arrow_function' || valueNode.type === 'function') && isValidSymbolName(nameNode.text)) {
          ...
          symbols.push({ name: currentActiveSymbol as string, kind: 'function', line: node.startPosition.row + 1, byteStart: node.startIndex, byteEnd: node.endIndex, snippet: capSnippet(content.substring(node.startIndex, node.endIndex)) });
        }
      }
      ...
      if (node.type === 'call_expression') {
        ...
        if (fnNode) {
          ...
          if (fnNode.type === 'member_expression') {
            ...
            if (propNode) calledName = propNode.text;
          }
          ...
          if (fnNode.text === 'require' || fnNode.text === 'import') {
            ...
            if (first && first.type === 'string') imports.push(unquote(first.text));
            else blindspots.push(makeBlindspot('DYNAMIC_IMPORT', 'warn', `Dynamic import/require near line ${node.startPosition.row + 1}`, 'runtime value needed for exact edge'));
          }
        }
      }
      ...
      if (node.type === 'import_statement' || node.type === 'export_statement') {
        ...
        if (sourceNode && sourceNode.type === 'string') imports.push(unquote(sourceNode.text));
      }
    ...
    for (const s of fallback.symbols) {
      ...
      const key = `${s.kind}:${s.name}:${s.line}`;
      if (!seen.has(key)) symbols.push(s);
    }
    imports.push(...fallback.imports);
    callEdges.push(...fallback.callEdges);
    for (const b of fallback.blindspots.filter((x) => !x.startsWith('DEGRADED_PARSE'))) blindspots.push(b);
