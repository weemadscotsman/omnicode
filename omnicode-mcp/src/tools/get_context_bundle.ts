...
export async function getContextBundle(
  ...
  format: OcapFormat = 'auto'
) {
  const db = initDb(repoPath);

  // 1. Get the target symbol
  const symbolRow = db.prepare(`SELECT id, file_id, name, kind, snippet, line, importance_score FROM symbols WHERE name = ? COLLATE NOCASE`).get(symbolName) as any;
  if (!symbolRow) {
    return { result: `Symbol ${symbolName} not found in index.` };
  }

  const fileRow = db.prepare(`SELECT path FROM files WHERE id = ?`).get(symbolRow.file_id) as any;
  ...
  const resolvedFormat = resolveOcapFormat(format, candidates.length);

  if (resolvedFormat === 'ocap') {
    const ocap = makeOcapBuilder('get_context_bundle', ['rel', 'name', 'kind', 'path', 'line', 'importance']);
