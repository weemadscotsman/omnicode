...
const sliceCache = new Map<string, { time: number, data: any }>();

export async function getFileSlice(repoPath: string, filePath: string, startLine: number, endLine: number, max_tokens: number = 4000) {
  ...
  try {
    ...
    if (usedTokens > max_tokens) {
      ...
      `).all(file.id, startLine, endLine) as any[];
