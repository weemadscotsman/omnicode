import { initDb, getIndexMeta, getFileProtection } from '../store/db';
import { getResolutionForPath } from '../engine/resolution';
...
export type RepairIntent = 'dead_code' | 'cyclic_deps' | 'god_object' | 'long_file' | 'rename' | 'delete' | 'general';
...
export async function repairPlan(repoPath: string, options: RepairPlanOptions = {}) {
  ...
  try {
    ...
    markdown += `${formatRepoVisionGate(gate)}\n\n`;
    ...
    if (partialIndex && destructive) {
      const stop = getIndexMeta(db, 'scan_stop_reason') || 'a scan guard';
      ...
      markdown += `Next proof needed: re-run index_project with raised limits (maxFiles/maxBytes/maxScanMs) until scan_stop_reason = complete.\n`;
      return writeHandoff(repoPath, markdown, options.output_path);
    }
    ...
    if (destructive && options.target) {
      const res = getResolutionForPath(db, repoPath, options.target);
    ...
    if (destructive && options.target) {
      const targetPath = path.isAbsolute(options.target) ? options.target : path.join(repoPath, options.target);
      const prot = getFileProtection(db, targetPath);
      if (prot.protected) {
    ...
    const hotspotRows = db.prepare(`
      ...
      LIMIT 8
    `).all() as Array<{ name: string; kind: string; path: string; score: number }>;
...
function writeHandoff(repoPath: string, markdown: string, outputPath?: string) {
  const outPath = outputPath
    ? path.resolve(repoPath, outputPath)
    : path.join(repoPath, '.omnicode', 'NO_SPAGHETT_REPAIR_HANDOFF.md');
  ...
  try {
    appendMemoryEvent(repoPath, {
      ...
      summary: `No Spaghett repair handoff written to ${path.relative(repoPath, resolved)}`,
      source: 'repair_plan',
  ...
  return {
    result:
      `No Spaghett generated an advisory handoff file only.\n` +
