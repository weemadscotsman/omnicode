...
exports.deadCodeScan = deadCodeScan;
const db_1 = require("../store/db");
async function deadCodeScan(repoPath) {
  ...
  `).all();
    ...
    for (const s of symbols) {
        ...
        if (s.path.includes('test') || s.path.includes('spec'))
            fileImportance = 1;
        else if (s.path.includes('utils') || s.path.includes('helpers'))
            fileImportance = 3;
        else if (s.path.includes('core') || s.path.includes('engine'))
            fileImportance = 8;
