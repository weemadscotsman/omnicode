...
exports.auditAgentConfig = auditAgentConfig;
...
async function readFileSafe(p) {
    ...
    if (!exists)
        return { path: p, kind: 'missing', content: '', exists: false };
    const content = await promises_1.default.readFile(p, 'utf8');
    const ext = path_1.default.extname(p).toLowerCase();
    const kind = ext === '.json' ? 'json' : ext === '.toml' ? 'toml' : ext === '.md' ? 'markdown' : 'text';
...
function extractToolMentions(text) {
    ...
    for (const m of bt) {
        const inner = m.slice(1, -1);
        if (/^[A-Z]/.test(inner) || inner.includes('__'))
            out.add(inner);
    }
    // snake_case identifiers
    const sc = text.match(/\b([a-z]+(?:_[a-z]+){1,4})\b/g) || [];
    for (const m of sc) {
        if (m.length >= 6 && m.length <= 32)
            out.add(m);
    }
...
async function discoverConfigFiles(target, home = HOME) {
    const candidates = [];
    if (target) {
        ...
        // Claude Code
        candidates.push(path_1.default.join(home, '.claude.json'));
        candidates.push(path_1.default.join(home, '.claude', 'CLAUDE.md'));
        candidates.push(path_1.default.join(home, '.claude', 'AGENTS.md'));
        candidates.push(path_1.default.join(home, '.claude', 'settings.json'));
        // Codex
        candidates.push(path_1.default.join(home, '.codex', 'config.toml'));
        candidates.push(path_1.default.join(home, '.codex', 'AGENTS.md'));
        // Cursor
        candidates.push(path_1.default.join(home, '.cursorrules'));
        candidates.push(path_1.default.join(home, '.cursor', 'rules'));
        // Cline
        candidates.push(path_1.default.join(home, '.clinerules'));
        // Windsurf
        candidates.push(path_1.default.join(home, '.windsurfrules'));
        // Copilot
        candidates.push(path_1.default.join(home, '.github', 'copilot-instructions.md'));
        // Project-level
        candidates.push(path_1.default.join(process.cwd(), 'CLAUDE.md'));
        candidates.push(path_1.default.join(process.cwd(), 'AGENTS.md'));
        candidates.push(path_1.default.join(process.cwd(), '.cursorrules'));
        // Claude Desktop
        candidates.push(path_1.default.join(APPDATA, 'Claude', 'claude_desktop_config.json'));
    }
    const out = [];
...
async function auditOne(file) {
    ...
    if (file.kind === 'json') {
        ...
        if (servers.length > 0) {
            ...
            const policySiblings = [
                file.path.replace(/[\\/][^\\/]+$/, '') + path_1.default.sep + 'CLAUDE.md',
                file.path.replace(/[\\/][^\\/]+$/, '') + path_1.default.sep + 'AGENTS.md',
                path_1.default.join(HOME, '.claude', 'CLAUDE.md'),
                path_1.default.join(HOME, '.claude', 'AGENTS.md'),
            ];
            let hasPolicy = false;
            ...
            for (const srv of servers) {
                ...
                if (srv.command === 'cmd' && srv.args.length > 0) {
                    ...
                    if (launcher.endsWith('.cmd') && !fs_1.default.existsSync(launcher)) {
                        findings.push({
                            category: 'STALE_PATH',
                            severity: 'high',
                            location: `${file.path} → ${srv.name}`,
    ...
    if (file.kind === 'markdown' || file.kind === 'text') {
        ...
        for (const m of mentions) {
            if (/^(true|false|null|undefined|this|that|args|kwargs|var|let|const|def|class|function|return|if|else|for|while|import|from|export|require)$/i.test(m))
                continue;
            if (m.length < 6)
                continue;
            findings.push({
    ...
    if (file.kind === 'toml') {
        ...
        for (const p of paths) {
            if (p.includes('${') || p.includes('{{') || p.includes('<') || p.includes('*'))
                continue;
            if (p.startsWith('http') || p.startsWith('/github.com'))
                continue;
            if (!fs_1.default.existsSync(p)) {
                findings.push({
                    category: 'STALE_PATH',
...
async function auditAgentConfig(opts = {}) {
    ...
    lines.push(`Files scanned: ${existing.map((f) => path_1.default.basename(f.path)).join(', ')}`);
    if (missingPolicyHint.length > 0) {
        lines.push(`Files NOT found (informational): ${missingPolicyHint.map((f) => path_1.default.basename(f.path)).join(', ')}`);
    }
    lines.push('');
    if (deduped.length === 0) {
        ...
        for (const [cat, fs] of Object.entries(grouped)) {
            ...
            for (const f of fs) {
                lines.push(`  • [${f.severity}] ${path_1.default.basename(f.location)}`);
