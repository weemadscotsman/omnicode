...
exports.deriveResolution = deriveResolution;
exports.classifyResolution = classifyResolution;
exports.getResolutionCoverage = getResolutionCoverage;
exports.getResolutionForPath = getResolutionForPath;
...
const SOURCE_LIKE_KINDS = new Set(['source', 'route', 'test']);
...
function classifyFileKind(relPath) {
    ...
    if (SECRET_NAME.test(lower) || (SECRET_HINT.test(lower) && (ext === '.json' || ext === '.txt' || ext === '')))
        return 'secret_risk';
    if (GENERATED_HINT.test(lower) || LOCK_NAME.test(lower))
        return 'generated';
    if (ABI_NAME.test(lower))
        return 'abi_runtime';
    if (SCHEMA_NAME.test(lower) || SCHEMA_EXT.has(ext))
        return 'schema';
    if (TEST_NAME.test(lower))
        return 'test';
    if (ROUTE_NAME.test(lower) && SOURCE_EXT.has(ext))
        return 'route';
    if (CONFIG_NAME.test(lower) || (CONFIG_EXT.has(ext) && !SOURCE_EXT.has(ext)))
        return 'config';
    if (SCRIPT_EXT.has(ext))
        return 'config'; // ops/shell scripts — classified, not "unknown"
    if (DOC_EXT.has(ext))
        return 'docs';
...
function resolverRequirementFor(ext) {
    const map = {
...
function grammarRequirementFor(ext) {
    const lang = {
...
function deriveResolution(f) {
    ...
    if (f.parser_mode === 'skipped') {
        ...
        if (kind === 'asset' || kind === 'binary')
            return row(f, kind, 'resolved_metadata', `${kind} file — classified, not source`, 'asset_classifier', 0.85);
        if (kind === 'docs')
            return row(f, kind, 'resolved_metadata', 'documentation — recorded as metadata', 'doc_classifier', 0.85);
        if (kind === 'config' || kind === 'schema' || kind === 'abi_runtime')
            return row(f, kind, 'resolved_metadata', `${kind} file — recorded; structured resolver can deepen this`, 'metadata_classifier', 0.75);
    ...
    if (f.hard_fail > 0 && f.symbol_count === 0) {
        if (isSource)
            return row(f, kind, 'resolver_missing', `no working parser/grammar: needs ${resolverRequirementFor(ext)}`, 'none', 0.25);
    ...
    if (f.symbol_count > 0 || f.total_imports > 0 || f.parser_mode === 'tree-sitter' || f.parser_mode === 'fallback') {
        ...
        if (partial) {
            const why = [];
            if (f.parser_mode === 'fallback')
                why.push('fallback parser');
            if (localGaps > 0)
                why.push(`${localGaps} unresolved local import(s)`);
            if (f.parse_error > 0)
                why.push('parse warnings');
            if (f.parse_quality < 0.85)
                why.push(`quality ${f.parse_quality.toFixed(2)}`);
    ...
    if (isSource)
        return row(f, kind, 'resolver_missing', `fallback produced no symbols: needs ${grammarRequirementFor(ext)}`, 'none', 0.2);
    return row(f, kind, 'resolved_metadata', 'no extractable symbols (empty/comment-only) — recorded', f.parser_mode, 0.7);
}
function row(f, kind, state, reason, resolver, confidence) {
...
function classifyResolution(db, repoPath) {
  ...
  `);
    const tx = db.transaction(() => {
        for (const fact of facts) {
            const rel = path_1.default.isAbsolute(fact.path) ? path_1.default.relative(repoPath, fact.path) : fact.path;
            upsert.run(deriveResolution({ ...fact, path: rel }));
        }
    });
    tx();
}
const SOURCE_LIKE = ['source', 'route', 'test'];
const BLOCKING_STATES = ['resolver_missing', 'requires_runtime', 'failed_with_reason'];
function getResolutionCoverage(db) {
    const rows = db.prepare(`SELECT r.kind, r.state FROM resolution r`).all();
    const byState = {};
    const byKind = {};
    ...
    for (const r of rows) {
        ...
        if (isSource) {
            sourceFiles++;
            if (r.state === 'resolved_full' || r.state === 'resolved_partial')
                sourceResolved++;
            if (r.state === 'resolver_missing')
                unresolvedSource++;
        }
        else if (r.state !== 'unsafe_excluded') {
            artifactsClassified++;
        }
        if (r.state === 'requires_runtime')
            runtimeRequired++;
        if (BLOCKING_STATES.includes(r.state))
            blockingRepairGaps++;
        if (r.kind === 'unknown' && r.state !== 'resolved_metadata' && r.state !== 'unsafe_excluded' && r.state !== 'generated_excluded')
            unknown++;
    }
    return {
        total: rows.length,
...
function getResolutionForPath(db, repoPath, filePath) {
    const abs = path_1.default.isAbsolute(filePath) ? filePath : path_1.default.join(repoPath, filePath);
    const r = db.prepare(`
