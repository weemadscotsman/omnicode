...
// Blindspot Explorer: turns "high blindspot rate" from a scary number into an
// actionable, explained list. A blindspot is an honest "I see this but can't
// statically resolve it" flag — it does NOT force a raw-source fallback, so it
// never costs token savings. This tool says exactly WHAT is unresolved and WHY.

interface ParsedReason { code: string; severity: string; message: string; hint: string; }

function splitReason(reason: string): ParsedReason {
  ...
  if (parts.length >= 3) return { code: parts[0], severity: parts[1], message: parts[2], hint: parts.slice(3).join('|').replace(/^hint=/, '') };
  if (reason.startsWith('Parse error')) return { code: 'PARSE_ERROR', severity: 'warn', message: reason, hint: 'partial/unsupported syntax or generated code' };
...
function specifierKind(spec: string): { kind: string; explain: string } {
  if (spec.startsWith('@/') || spec.startsWith('#')) return { kind: 'unresolved_alias', explain: 'Path alias not wired up. → add it to tsconfig/jsconfig "paths" or your bundler resolver.' };
  if (spec.startsWith('./') || spec.startsWith('../') || spec.startsWith('.')) return { kind: 'unresolved_relative', explain: 'A local path that did not resolve to an indexed file. → broken import, a generated/missing target, or a file the scanner excluded.' };
  if (spec.startsWith('/')) return { kind: 'unresolved_absolute', explain: 'Absolute path import — environment-specific. → usually external to the repo.' };
...
export async function blindspotExplorer(repoPath: string, opts: { top?: number; explain?: boolean } = {}) {
  const top = opts.top ?? 20;
  const explain = !!opts.explain;
  const db = initDb(repoPath);

  const totalFiles = (db.prepare(`SELECT COUNT(*) c FROM files`).get() as { c: number }).c;
  const totalBlind = (db.prepare(`SELECT COUNT(*) c FROM blindspots`).get() as { c: number }).c;
  const filesWithBlind = (db.prepare(`SELECT COUNT(DISTINCT file_id) c FROM blindspots`).get() as { c: number }).c;

  const blindRows = db.prepare(`SELECT b.reason, f.path FROM blindspots b JOIN files f ON f.id = b.file_id`).all() as Array<{ reason: string; path: string }>;
  const byClass: Record<string, { count: number; severity: string; hint: string; examples: string[] }> = {};
  for (const r of blindRows) {
    const p = splitReason(r.reason);
    if (!byClass[p.code]) byClass[p.code] = { count: 0, severity: p.severity, hint: p.hint, examples: [] };
    byClass[p.code].count++;
    if (byClass[p.code].examples.length < 3) byClass[p.code].examples.push(path.relative(repoPath, r.path));
  }

  const unresolved = db.prepare(`
    SELECT specifier, COUNT(*) c FROM imports WHERE to_file_id IS NULL
    GROUP BY specifier ORDER BY c DESC LIMIT ?
  `).all(top) as Array<{ specifier: string; c: number }>;
  db.close();
  ...
  if (!classes.length) out += `  none\n`;
  for (const [code, g] of classes) {
    out += `- ${code}: ${g.count} · severity ${g.severity}\n`;
    if (explain && CLASS_EXPLAIN[code]) out += `    why: ${CLASS_EXPLAIN[code]}\n`;
    else out += `    ${g.hint}\n`;
    if (g.examples.length) out += `    e.g. ${g.examples.join(', ')}\n`;
  }

  out += `\nTop ${top} unresolved references (imports the graph couldn't bind):\n`;
  if (!unresolved.length) out += `  none — every import resolved\n`;
