...
export async function indexProject(repoPath: string, onProgress?: (current: number, total: number) => void, options: IndexOptions = {}) {
  ...
  const finalStopReason = indexIncomplete ? 'index_incomplete' : (scanResult.stats.stopReason || 'complete');
  setIndexMeta(db, 'partial_index', finalStopReason === 'complete' ? '0' : '1');
  ...
  const result = {
    ...
    partialIndex: finalStopReason !== 'complete',
    resume: {
  ...
  try {
    appendMemoryEvent(repoPath, {
      type: 'session.note',
      summary: `Index completed: ${totalFiles.count} files in index, stop reason ${finalStopReason}.`,
      source: 'index_project',
      data: {
        subtype: 'index.completed',
        indexed_files: totalFiles.count,
        scanned_files: files.length,
        partial_index: finalStopReason !== 'complete',
