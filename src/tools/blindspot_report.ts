import { initDb, getConnectivityCounts, getStagedFiles } from '../store/db';
...
function splitReason(reason: string) {
  ...
  if (reason.startsWith('Parse error near line')) return { code: 'PARSE_ERROR', severity: 'warn', message: reason, hint: 'inspect syntax/generated file' };
  if (reason.startsWith('Failed to parse')) return { code: 'PARSE_FAILURE', severity: 'error', message: reason, hint: 'fallback parser or native grammar needed' };
  if (reason.includes('Unsupported extension')) return { code: 'UNSUPPORTED_EXTENSION', severity: 'error', message: reason, hint: 'add scanner/parser support or ignore file type' };
  return { code: 'UNKNOWN_BLINDSPOT', severity: 'warn', message: reason, hint: 'inspect manually' };
}

export async function blindspotReport(repoPath: string) {
  ...
  const unresolvedImports = db.prepare(`
    ...
    ORDER BY f.path
    LIMIT 200
  `).all() as { specifier: string; file_path: string }[];
  ...
  const blindspots = db.prepare(`
    ...
    ORDER BY f.path
  `).all() as { reason: string; file_path: string; lang: string; parser_mode: string; parse_quality: number; language_name: string }[];
  ...
  const grouped: Record<string, { count: number; severity: string; hint: string; examples: string[] }> = {};
  for (const b of blindspots) {
    const parsed = splitReason(b.reason);
    const key = parsed.code;
    if (!grouped[key]) grouped[key] = { count: 0, severity: parsed.severity, hint: parsed.hint, examples: [] };
    grouped[key].count++;
    if (grouped[key].examples.length < 5) grouped[key].examples.push(`${path.relative(repoPath, b.file_path)} :: ${parsed.message}`);
  }
  ...
  report += `${formatRepoVisionGate(gate)}\n\n`;
  report += `Parser coverage by language:\n`;
  for (const row of parserRows) report += `- ${row.language_name || 'unknown'} / ${row.parser_mode}: ${row.count} files · quality ${row.quality ?? 0}\n`;

  report += `\nBlindspot classes:\n`;
  if (Object.keys(grouped).length === 0) report += `- none recorded\n`;
  for (const [code, g] of Object.entries(grouped).sort((a,b) => b[1].count - a[1].count)) {
    ...
    for (const ex of g.examples) report += `  • ${ex}\n`;
  }
  ...
  const conn = getConnectivityCounts(db);
  ...
  if (Object.keys(conn).length === 0) report += `- not computed (re-index to populate)\n`;
  else for (const k of connOrder) if (conn[k]) report += `- ${k.toLowerCase()}: ${conn[k]}\n`;

  const staged = getStagedFiles(db, 10);
  if (staged.length > 0) {
    ...
    for (const s of staged) {
      ...
      report += `- ${path.relative(repoPath, s.path)} · exports ${s.export_count} · touched ${s.touch_count}x · intent ${s.intent_score.toFixed(2)}${flag}\n`;
    }
    report += `Note: staged files are never auto-deleted on graph evidence alone; intent guards them.\n`;
  }

  report += `\nUnresolved import categories:\n`;
  if (Object.keys(unresolvedByKind).length === 0) report += `- none in sample\n`;
  for (const [kind, count] of Object.entries(unresolvedByKind)) report += `- ${kind}: ${count}\n`;

  report += `\nNext fixes:\n`;
  if ((fileStats.fallback_files || 0) > 0) report += `- Install native tree-sitter grammars for fallback languages to raise semantic confidence.\n`;
  if ((unresolvedByKind['alias_unresolved'] || 0) > 0) report += `- Add/verify tsconfig/vite/webpack alias resolver for @/ and workspace aliases.\n`;
  if ((grouped['DYNAMIC_RUNTIME_REFERENCE']?.count || 0) > 0) report += `- Use runtime_trace/LSP/ABI resolver before repair actions that touch dynamic symbols.\n`;
  if ((grouped['PARSE_ERROR']?.count || 0) > 0) report += `- Inspect generated/partial files or add grammar support for syntax variant.\n`;
  if (risk === 'LOW') report += `- Repo vision is good enough for token-saving queries; keep blindspot warnings attached to repair plans.\n`;
