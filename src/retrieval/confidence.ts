...
export function computeConfidence(
  scores: number[],
  opts: { isStale?: boolean; hasIdentityMatch?: boolean | null } = {}
): ConfidenceResult {
  ...
  const identity =
    hasIdentityMatch === true ? 1.0 : hasIdentityMatch === null ? 0.7 : 0.6;
  const freshness = isStale ? 0.6 : 1.0;

  const components: ConfidenceComponents = { gap, strength, identity, freshness };
