...
export type OcapFormat = 'text' | 'ocap' | 'auto';

export interface OcapInternBucket {
  name: string;
  values: string[];
  index: Map<string, number>;
}

export interface OcapPayload {
  tool: string;
  keys: string[];
  buckets: OcapInternBucket[];
  rows: Array<Array<string | number | null>>;
  footer?: Record<string, string | number | boolean | null>;
}

export interface OcapSerializeOptions {
  indentBuckets?: boolean;
}
...
function internBucket(name: string): OcapInternBucket {
  return { name, values: [], index: new Map() };
}

export function makeOcapBuilder(tool: string, keys: string[]): {
  payload: OcapPayload;
  intern: (bucket: string, value: string) => number;
  push: (row: Array<string | number | null>) => void;
  setFooter: (key: string, value: string | number | boolean | null) => void;
  toText: (opts?: OcapSerializeOptions) => string;
} {
  const payload: OcapPayload = {
    tool,
    keys,
    buckets: [],
    rows: [],
  ...
  const bucketByName = new Map<string, OcapInternBucket>();

  const intern = (bucket: string, value: string): number => {
  ...
  const push = (row: Array<string | number | null>) => {
    payload.rows.push(row);
  };

  const setFooter = (key: string, value: string | number | boolean | null) => {
    if (!payload.footer) payload.footer = {};
    payload.footer[key] = value;
  };

  const toText = (_opts?: OcapSerializeOptions): string => {
    const out: string[] = [];
    ...
    if (payload.footer) {
      const footerEntries = Object.entries(payload.footer)
        ...
        .join(' ');
      if (footerEntries) out.push(`${FOOTER_PREFIX} ${footerEntries}`);
    }
    return out.join('\n');
  };
...
export function shouldUseOcap(format: OcapFormat | undefined, rowCount: number): boolean {
  if (format === 'ocap') return true;
...
export function resolveOcapFormat(format: OcapFormat | undefined, rowCount: number): 'ocap' | 'text' {
