...
export type RepoVisionRisk = 'LOW' | 'MEDIUM' | 'HIGH';

export interface RepoVisionGate {
  risk: RepoVisionRisk;
...
export function getRepoVisionGate(db: Database.Database): RepoVisionGate {
  ...
  if (files === 0) reasons.push('no indexed files');
  if (stats.unparsedFiles > 0) reasons.push(`${stats.unparsedFiles} unparsed files`);
  if (stats.parserErrors > 0) reasons.push(`${stats.parserErrors} parser/error blindspots`);
  if (stats.dynamicRuntimeWarnings > 0) reasons.push(`${stats.dynamicRuntimeWarnings} dynamic runtime warnings`);
  if (stats.unresolvedAliases > 0) reasons.push(`${stats.unresolvedAliases} unresolved aliases/import maps`);
  if (stats.unresolvedRelativeImports > 0) reasons.push(`${stats.unresolvedRelativeImports} unresolved relative imports`);
  if (stats.fallbackFiles > 0) reasons.push(`${stats.fallbackFiles} fallback-parsed files`);
  if (avgQuality > 0 && avgQuality < 0.55) reasons.push(`low average parse quality ${avgQuality}`);
  if (stats.blindspotRate > 20) reasons.push(`blindspot rate ${stats.blindspotRate.toFixed(1)}%`);
  ...
  const risk: RepoVisionRisk = high ? 'HIGH' : medium ? 'MEDIUM' : 'LOW';
  return {
    risk,
...
export function formatRepoVisionGate(gate: RepoVisionGate): string {
