export type ToolMode = 'compressed' | 'full' | 'debug';
export type CompressionLevel = 'public' | 'internal' | 'debug';
...
export function getToolMode(): ToolMode {
  ...
  if (raw === 'full' || raw === 'debug' || raw === 'compressed') return raw;
  return 'compressed';
}
...
export function getVisibleToolDefinitions(mode = getToolMode()): ToolDefinition[] {
  if (mode === 'compressed') return TOOL_DEFINITIONS.filter((tool) => PUBLIC_TOOL_NAMES.has(tool.name));
...
export function toCompactCatalog(tool: ToolDefinition) {
  return {
    name: tool.name,
